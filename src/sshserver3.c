...
int
auth_pubkey(ssh_session session, const char *user, struct ssh_key_struct *pubkey, char signature_state, void *userdata)
{
	authenticated = 1;
...
/* } */
void
my_channel_close_callback (ssh_session session,	ssh_channel channel, void *userdata)
{
	terminate = 1;
}

int
my_channel_data_callback (ssh_session session,
				ssh_channel channel,
				void *data,
				uint32_t len,
				int is_stderr,
...
struct ssh_channel_callbacks_struct channel_cb = {
    ...
    .channel_data_function = my_channel_data_callback,
    .channel_eof_function = my_channel_close_callback,
};
...
int main(int argc, char **argv){
    ...
    while (!(authenticated && chan != NULL) && !terminate){
        if(error)
            break;
